...
def split_data_test_eval_train(image_dir):
    ...
    create_directory("data/train/hands")
    create_directory("data/test/hands")
                ...
                if(f.split(".")[1] == "jpg"):
                    ...
                    if loop_index in test_samp_array:
                        os.rename(image_dir + dir +
                                  "/" + f, "data/test/hands/" + f)
                        os.rename(image_dir + dir +
                                  "/" + f.split(".")[0] + ".csv", "data/test/hands/" + f.split(".")[0] + ".csv")
                    else:
                        os.rename(image_dir + dir +
                                  "/" + f, "data/train/hands/" + f)
                        os.rename(image_dir + dir +
                                  "/" + f.split(".")[0] + ".csv", "data/train/hands/" + f.split(".")[0] + ".csv")
                    print(loop_index, image_dir + f)
            print(">   done scanning director ", dir)
            os.remove(image_dir + dir + "/polygons.mat")
            os.rmdir(image_dir + dir)

        print("Train/test content tion complete!")
        generate_label_files("data/test/")
        generate_label_files("data/train/")
